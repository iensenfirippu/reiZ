...
if (defined('reiZ') or exit(1))
{
	class BlogPost extends reiZ_DatabaseObject
	{
		...
		public function GetFullText($final=true)	{ $value = $this->_fulltext != EMPTYSTRING ? $this->_fulltext : $this->_text;
																	  return $final == true ? ExecuteCommands($value) : $value; }
		...
		{
			$this->SetShortText(String_Ellipsize(String_StripHtml($value), 255));
		...
		public function Save()
		{
			...
			if ($this->GetPostID() == null)
			{
				...
				if (SetAndNotNull($array, $this::$_fields[0]))
				{
		...
		{
			$result = (SetAndNumerical($id)) ? self::Load(array(array(self::$_dbtable.'.'.self::$_fields[0], DB_Operator::Is, $id))) : null;
			if (is_int($count) && $result != null) { $count = 1; }
			return $result;
		}
		
		public static function LoadNewest($category, $tag, $limit_start, $limit_amount, &$count=null)
		{
			$posts = array();
			$conditions = array();
			if ($category instanceof BlogCategory) { $conditions[] = array(self::$_fields[1], DBOP::Is, $category->GetID()); }
			if ($tag instanceof BlogTag) { $conditions[] = array('blogposttag.t_id', DBOP::Is, $tag->GetID()); }
			
			if (sizeof($conditions) > 0) {
				$query = parent::PrepareLoadAllQuery($conditions);
				if ($tag instanceof BlogTag) { self::InnerJoinBlogTag($query); }
				$rows = parent::RunLoadAllQuery($query, $limit_start, $limit_amount, 'p_id', DBOD::Desc, $count);
				foreach ($rows as $row) { $posts[] = new BlogPost($row); }
			} else {
				$posts = self::LoadAll(null, $limit_start, $limit_amount, 'p_id', DBOD::Desc, $count);
			}
			return $posts;
		}
