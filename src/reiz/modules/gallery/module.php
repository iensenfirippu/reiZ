	...
	class GalleryModule extends Module
	{
		...
		public function Initialize()
		{
			if (!$this->_initialized)
			{
				...
				{
					$GLOBALS['HTML']->AddStyleSheet('common/styles/lightbox.css');
					$GLOBALS['HTML']->AddJavascript('common/scripts/jquery-1.10.2.min.js');
					$GLOBALS['HTML']->AddJavascript('common/scripts/lightbox-2.6.min.js');
				}
			}
		}
		...
		private function GenerateHtml()
		{
			...
			else { Redirect((new URL(URLPAGE.GetSafeArgument(GETPAGE)))->GetAbsolutePath()); }
			// Else redirect to previous gallery (loops until an existing gallery is found, or the max redirect is reached)
			//else { Redirect(URLPAGE.GetSafeArgument(GETPAGE).URLARGS.substr($url, 0, strrpos( $url, SINGLESLASH))); }
		}
		...
		private function GetHtml_Preview($name, $args)
		{
			...
			if ($folder != null)
			{
				...
				{
					$images = array();
					if (SetAndNotNull($args, 0))
					{
						if (is_numeric($args[0]) && Array_LongerThan($folder->GetFiles(), $args[0] -1)) { $images[] = $folder->GetFiles()[$args[0] -1]; }
						elseif (is_string($args[0]))
						{
							if (String_Contains($args[0], SINGLECOMMA))
							{
								$indexes = explode(SINGLECOMMA, $args[0]);
								foreach ($indexes as $index) {
									if (Array_LongerThan($folder->GetFiles(), $index -1)) {
										$images[] = $folder->GetFiles()[$index -1];
									}
								}
							}
							elseif (String_Contains($args[0], '-'))
							{
								...
								if (is_array($limits) && sizeof($limits) == 2 && is_numeric($limits[0]) && is_numeric($limits[1]))
								{
									for ($i = $limits[0]; $i <= $limits[1]; $i++) {
										if (Array_LongerThan($folder->GetFiles(), $i -1)) {
											$images[] = $folder->GetFiles()[$i -1];
										}
									}
								}
							}
		...
		public function GetSettings()
		{
			...
			$settings->Add('GALLERYDIR',					'File directory',				ST::StringValue,		'content/gallery');
			$settings->Add('GALLERYTHUMBSDIR',					'File directory',				ST::StringValue,		'content/gallery-thumbs');
			$settings->Add('GALLERYNOPREVIEW',			'Default image',				ST::StringValue,		'common/images/reiz/nopreview.jpg');
