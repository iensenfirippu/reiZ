...
if (defined('reiZ') or exit(1))
{
	...
	class HtmlAttributes extends ListObject
	{
		...
		{
			$result = EMPTYSTRING;
			if (SetAndNotNull($this->_list) && is_array($this->_list)) {
				ksort($this->_list);
		...
		/**
		 * Add an class name to the attributes list
		 * @param var $class The class name to add
		 **/
		public function AddClass($class)
		{
			if (is_string($class)) {
				if (array_key_exists('class', $this->_list)) {
					$this->_list['class'] .= SINGLESPACE.$class;
				} else {
					$this->_list['class'] = $class;
				}
			}
		}
