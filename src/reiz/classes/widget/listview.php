...
if (defined('reiZ') or exit(1))
{
	...
	class RTK_Listview extends HtmlElement
	{
		...
		private function GetRowClass($i, $isheader=false)
		{
			...
			$value = 'row';
			if ($i === 0) { $value .= ' first'; }
			if ($isheader) { $value .= ' header'; }
			else {
				Boolean_Flip($this->_alternaterow);
				if ($this->_alternaterow) { $value .= ' alt'; }
			}
			if ($i === null) { $value .= ' last'; }
		...
		{
			$value = 'lv_cell';
			Boolean_Flip($this->_alternatecell);
		...
		private function AppendRow($row, $i)
		{
			...
			for ($i = 0; $i <= $rowsize; $i++) {
				if (is_a($row[$i], 'HtmlElement')) {
					$line->AddChild(new HtmlElement('td', array('class' => $this->GetCellClass($i, $rowsize)), null, $row[$i]));
				} else {
